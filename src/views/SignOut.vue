    ...
    <div class="m-5 shadow p-3">
      ...
      <form class="flex-col flex" @submit.prevent="onSubmitMultiple">
        <v-form-control label="Upload CSV of 'assignedTo, location, number'" label-for="sign-out-csv">
          <input v-model="csvfile" type="file" name="sign-out-csv" id="sign-out-csv" accept="text/csv">
        </v-form-control>
...
export default {
  name: 'SignOut',
  components: { VScanner, VFormControl, VAutocomplete },
  data() {
    return {
      number: '',
      assignedTo: '',
    ...
    async onSubmit() {
      try {
        ...
        if (number.length === 0 || assignedTo.length === 0 || location.length === 0) {
          this.$toasted.error('Missing information')
          return
        }
        this.loading = true
