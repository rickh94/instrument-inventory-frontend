    ...
    <div class="m-5 shadow p-3 h-auto">
      ...
      <div class="flex items-start justify-between mt-2 mb-1">
        <div class="border-b focus-within:border-purple-300 border-gray-800 w-24 flex-grow flex items-center space-between">
          ...
          >
          <button class="appearance-none" title="Scan Barcode" @click="scanner = !scanner">
...
import { SEARCH_OUTCOMES, searchForInstrument } from "@/services/searchForInstrument";
...
export default {
  ...
  methods: {
    ...
    async onSubmit() {
      ...
      this.loading = true;
      const [searchOutcome, result] = await searchForInstrument(this.searchTerm, this.showArchived);
      switch (searchOutcome) {
        case SEARCH_OUTCOMES.foundOne:
          this.$toasted.info("Instrument found", { duration: 1000 });
          this.setCurrentInstrument(result);
          break;
        case SEARCH_OUTCOMES.foundMultiple:
          this.$toasted.info("Multiple instruments found", { duration: 1000 });
          this.setSearchResults(result);
          break;
        case SEARCH_OUTCOMES.foundOnlyArchived:
          this.$toasted.info("Only archived instruments found", { duration: 1000 });
          break;
        case SEARCH_OUTCOMES.numberNotFound:
          this.showNotFound = true;
          break;
        case SEARCH_OUTCOMES.nameNotFound:
          this.$toasted.error(`Could not find instrument assigned to ${result}`, { duration: 1000 });
          break;
        case SEARCH_OUTCOMES.error:
