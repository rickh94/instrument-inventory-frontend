    ...
    <div class="mt-4">
      ...
      <div class="flex flex-col sm:flex-row sm:items-end justify-between mt-2 mb-1">
        <v-form-control label="Size" label-for="size">
          <v-autocomplete id="size" v-model="nextSize" :options="acOptions.sizes"></v-autocomplete>
        </v-form-control>
        <v-form-control label="Type" label-for="type">
          <v-autocomplete id="type" v-model="nextType" :options="acOptions.types"></v-autocomplete>
        </v-form-control>
    ...
    async getNumber() {
      try {
        const instruments = await API.post("instrument-inventory", "filter",
          { body: { type: this.nextType, size: this.nextSize } },
        );
        const nextInstrumentNumber = instruments.map(ins => ins.number).sort().reverse()[0];
        if (nextInstrumentNumber === undefined) {
          this.$toasted.error("No instruments found in that type/size", { duration: 2000 });
          this.nextSize = "";
          this.nextType = "";
          return;
        }
        const [prefix, insNumber] = nextInstrumentNumber.split("-");
        const nextNumber = `${parseInt(insNumber) + 1}`.padStart(3, 0);
        this.setNewInstrumentNumber(`${prefix}-${nextNumber}`);
      } catch (e) {
        console.error(e);
        this.$toasted.error("Error: Something has gone wrong", { duration: 2000 });
      }
